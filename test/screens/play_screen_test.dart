      ...
      (tester) async {
        ...
        for (final side in PlayerSide.values) {
          for (final hand in HandPosition.values) {
            expect(
              tester.getSize(
                find.byKey(ValueKey('handArt-${side.name}-${hand.name}')),
              ),
              const Size(220, 220),
            );
            expect(
              tester.getSize(
                find.byKey(ValueKey('hand-${side.name}-${hand.name}')),
              ),
              const Size(220, 220),
            );
          }
        ...
        expect(
          tester.getCenter(find.byKey(const Key('hand-far-right'))).dx,
          greaterThan(
            tester.getCenter(find.byKey(const Key('hand-far-left'))).dx,
          ),
        );
